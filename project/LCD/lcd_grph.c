...
static unsigned int picBuffer[320*240];
static unsigned int picWidth, picHeight;
...
** 
******************************************************************************/
void lcd_picture(unsigned short x, unsigned short y, unsigned char *pPicture)
{
  unsigned int i = 4, px, py;
  unsigned char pixel332;

  for (py = y; py < y + pPicture[1]; py++)
  {
    for (px = x; px < x + pPicture[0]; px++)
    {
      pixel332 = pPicture[i++];

      if (pixel332 == 0xFF)
      {
        lcd_point(px,py, 0xFFFF);   
      }
      else
      {
        lcd_point(px,py, ((pixel332 & 0xE0) << 8) | ((pixel332 & 0x1C) << 6) | ((pixel332 & 0x03) << 3));  
      }
    }
  }
...
void loadPic(unsigned char *pPicture)
{
  unsigned int len, i;
  unsigned char pixel332;
  unsigned int r, g, b;
  ...
  picWidth = pPicture[0];
  picHeight = pPicture[1];

  len = picWidth * picHeight;

  for (i = 0; i < len; i++)
  {
    pixel332 = pPicture[i+4];
    ...
    picBuffer[i] = ((r << 16) | (g << 8) | b) & 0x00FFFFFF;
  }
...
void putPic(void)
{
  unsigned int originX, originY, x, y , i;
  unsigned int r, g, b;

  originX = (240-picWidth) >> 1;
  originY = (320-picHeight) >> 1;

  i = 0;

  for (y = originY; y < originY + picHeight; y++)
  {
    for (x = originX; x < originX + picWidth; x++)
    {
      r = picBuffer[i] >> 16;
      g = (picBuffer[i] >> 8) & 0xFF;
      b = picBuffer[i] & 0xFF;
      lcd_point(x, y, ((r << 8) & 0xF800) | ((g << 3) & 0x07E0) | ((b >> 3)));
      i++;
    }
  }
...
void * getPicAddr(void)
{
  return picBuffer;
}

unsigned int getPicWidth(void)
{
  return picWidth;
}

unsigned int getPicHeight(void)
{
  return picHeight;
}
